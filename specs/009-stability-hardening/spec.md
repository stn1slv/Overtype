...
- **FR-014** (H7): Unified-log output MUST be readable (correct format usage), and the documented debug logging mode MUST be reachable via a persistent switch; enabling it MUST present an explicit, user-visible privacy warning at launch (not only a log line), closing the corresponding Known Deviation in constitution v1.1.0.
