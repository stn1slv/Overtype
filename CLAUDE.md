...
- Build the distributable `.app` bundle (release build + Info.plist + ad-hoc codesign): `./scripts/build-app.sh` — produces `Overtype.app` in the repo root. The script also stamps the version into the bundle's `Info.plist` (version from `$OVERTYPE_VERSION` or the exact git tag at `HEAD`, build from `git rev-list --count HEAD`), editing only the bundle copy and never `Sources/Overtype/Resources/Info.plist`, whose values remain the fallback for an unstamped local build. The stamping must stay **before** the `codesign` call, which seals `Info.plist`. [Source: specs/006-settings-version-display]
