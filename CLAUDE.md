...
- **Evidence over assumption at the system boundary.** Accessibility/synthetic-event behavior is treated as empirically verified per app in `docs/compatibility.md`, not assumed. Accessibility API success return codes are not proof of effect. Any workaround for a platform quirk carries an inline comment naming the quirk and must not be "simplified" without a fresh diagnostic run. (Known real examples: setting selected-text in MS Teams returns success but changes nothing; a synthetic event source that inherits held modifiers turns typed characters into shortcuts.)
